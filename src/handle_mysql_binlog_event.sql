...
create function handle_mysql_event(
    inout event event
)
language plpgsql
    ...
    case
        when event.table_ = 'product' and event.name = 'writerows' then
            ...
            by_attr (id, attribute, rest) as (
              select id, j.* from mysql_product, jsonb_each(raw_values::jsonb) j
            ),
            ...
            select event.tenant, id, attribute, locale, channel, 'simple', value
            from by_locale;
